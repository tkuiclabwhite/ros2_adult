...
GRASP_OFFSET = np.array([0.0, 0.02, -0.11])  # -Z 15 cm in Rforarm2 frame
...
def fk(q):
    """Return (grasp_pos, R2, axis13_world) in base_link frame."""
    M = np.eye(4)
    axis13_world = None
...
if __name__ == "__main__":
    target = world_to_target(ball_h=0.9, shoulder_h=0.90, camera_h=1.05,
                             shoulder_right_of_cam=0.09, ball_front_of_cam=0.30)
    print("\n### Position-only IK ###")
