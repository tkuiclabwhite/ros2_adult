...
deep_y           : {self.deep_y}\n\
#===============A* 導航===============#\n\
Virtual X        : {self.robot.robot_x:.1f}\n\
Virtual Y        : {self.robot.robot_y:.1f}\n\
")
        sys.stdout.flush()
...
class Obs(API):
    def __init__(self):
        ...
        # ==========================================
        self.global_path = []
        self.robot_x = 450.0  # 起點在地圖中央 (300cm * scale 3 / 2)
        self.robot_y = 590.0  # 起點在地圖底部 (200cm * scale 3 - 10)
        self.load_path_file()
    ...
    def load_path_file(self):
        ...
        try:
            if os.path.exists(path_file):
                ...
                self.get_logger().info(f"成功從 {folder} 載入 {len(self.global_path)} 個 A* 座標點！")
            else:
                self.get_logger().error(f"找不到 {path_file}，將切換為純盲走避障模式。")
        except Exception as e:
            self.get_logger().error(f"讀取路徑檔失敗: {e}")
            ...
            #7.沒障礙物時========================================
                ...
                if self.calc.deep_y < 24 :
                    ...
                    if len(self.global_path) > 0:
                        # 1. 步態推算 (推算虛擬位置)
                        STEP_PIXELS = 1.0 # 每次迴圈 (0.05秒) 前進的像素量，需依實機速度微調
                        yaw_rad = math.radians(self.imu_rpy[2])
                        
                        # 地圖坐標系：0度直走時，Y應該減少(往上)，X不變
                        self.robot_x += STEP_PIXELS * math.sin(yaw_rad)
                        self.robot_y -= STEP_PIXELS * math.cos(yaw_rad)

                        # 2. 尋找 A* 前視點 (Lookahead point)
                        target_pt = self.global_path[-1] # 預設為終點
                        for pt in self.global_path:
                            # 計算虛擬機器人與路徑點的距離
                            dist = math.hypot(pt[0] - self.robot_x, pt[1] - self.robot_y)
                            if dist > 40: # 尋找前方約 40 像素的點
                                target_pt = pt
                                break
                        
                        # 3. 計算 A* 引力角度
                        dx = target_pt[0] - self.robot_x
                        dy = target_pt[1] - self.robot_y
                        # 轉換成與 IMU 一致的角度 (-dy 是因為地圖的 Y 軸是朝下的)
                        angle_to_target = math.degrees(math.atan2(dx, -dy))
                        
                        # 計算角度差並限制在 -180 ~ 180
                        angle_diff = angle_to_target - self.imu_rpy[2]
                        angle_diff = (angle_diff + 180) % 360 - 180
                        
                        # A* 目標引力 (P 控制權重)
                        theta_astar = angle_diff * 0.8 

                        # 4. 融合 obs.py 的斥力微調 (deep_x > 0 代表左邊空，要往左走)
                        # deep_x 本身帶有方向性，可直接作為修正參數
                        theta_repulsive = self.calc.deep_x * 0.5 

                        # 5. 最終輸出
                        final_theta = theta_astar + theta_repulsive
                        
                        # 限制最大轉向角，避免走路摔倒
                        final_theta = max(min(final_theta, 12), -12)

                        # 送出融合後的步態 (套用你原本定義的最大前進速度)
                        self.calc.speed_x = MAX_FORWARD_X
                        self.calc.y = 0
                        self.calc.theta = final_theta
                        self.sendContinuousValue(self.calc.speed_x, self.calc.y, int(self.calc.theta))
                        self.pre_act = 'astar_tracking'
                        
                    else:
                        # 如果找不到 path.txt，就執行你原本的純瞎走邏輯
