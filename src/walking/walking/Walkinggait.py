...
class WalkingGaitByLIPM:
    ...
    def process_sr_continuous(self):
        """連續步態 + IMU PD 即時平衡補正。
        進入此模式前，呼叫端需先設定 _locked_yaw = 當前 imu_yaw。
        IMU 值由走路節點在每幀寫入 _imu_pitch / _imu_roll / _imu_yaw。
        """
        self.process()
        ...
        locked_yaw     = getattr(self, "_locked_yaw", imu_yaw)
        imukp          = float(getattr(parameter, "imukp",          0.1))
        ...
        roll_corr  = max(-max_corr, min(max_corr,
                         imukp * roll_err + imukd * d_roll))

        # Yaw P (方向偏移補正 → var_theta_)
        yaw_err = imu_yaw - locked_yaw
        while yaw_err >  180.0: yaw_err -= 360.0
        while yaw_err < -180.0: yaw_err += 360.0
        yaw_corr = max(-max_corr, min(max_corr, yaw_kp * yaw_err))

        # 把 Pitch/Roll 補正疊加到 COM 參考位置，重新計算足端座標
        self.px_u += pitch_corr
        self.py_u += roll_corr
        self.coordinate_transformation()
        self.coordinate_offset()
